...
class DedicatedAuditorTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_cleanup_failure_preserves_original_startup_exception(self):
        with self.assertLogs("main", level="ERROR") as logs:
            await self.check_startup_cleanup(RuntimeError("cleanup failed"))
        self.assertIn("startup rollback", logs.output[-1])

    async def check_startup_cleanup(self, cleanup_error):
        startup_error = RuntimeError("startup failed")
        runtime = SimpleNamespace(model=Emitter(), aclose=AsyncMock(side_effect=cleanup_error))
        ...
        with patch("main.setup_logging"), \
             ...
             patch("main.AgentSession", side_effect=startup_error):
            with self.assertRaises(RuntimeError) as caught:
