...
def test_geo_validation_fallback_and_privileges():
    with migrated_database(Path(__file__).parents[1] / 'supabase/migrations', through=16) as url:
        with psycopg.connect(url, row_factory=dict_row) as db:
            _seed(db)
            db.execute("insert into company_hq(company,lat,lng,city,country,source) values ('Acme',32,34,'City','Country','https://example.test/hq | nominatim:osm:relation:1')")
...
def test_hq_eligibility_and_evidence_rejections():
    with migrated_database(Path(__file__).parents[1] / 'supabase/migrations', through=16) as url:
        with psycopg.connect(url, row_factory=dict_row) as db:
            _seed(db)
            db.execute("insert into company_hq(company,lat,lng,city,country,source) select company,32,34,'HQ City','HQ Country','https://example.test/hq | nominatim:osm:relation:1' from postings")
            db.execute('update postings set location=null,remote=false where id=%s', (IDS[1],))
            rows = db.execute('select * from get_job_geo(%s)', (IDS,)).fetchall()
            assert {str(r['posting_id']) for r in rows} == {IDS[0], IDS[1], IDS[2], IDS[4]}
            db.commit()
            for source in ('guess', 'http://example.test/hq | nominatim:osm:relation:1', 'https://example.test/hq', 'https://example.test/hq | guess'):
                with pytest.raises(psycopg.errors.CheckViolation):
                    db.execute('update company_hq set source=%s', (source,))
                db.rollback()
                with pytest.raises(psycopg.errors.CheckViolation):
                    db.execute("insert into posting_geo(posting_id,lat,lng,precision,source) values (%s,32,34,'hq',%s)", (IDS[3], source))
                db.rollback()
