...
def test_job_api_search_count_detail_and_security_contract() -> None:
    try:
        ...
        with database as url, psycopg.connect(url, row_factory=dict_row) as connection:
            ...
            routines = ("score_band(smallint)", "job_link_url(text,text)", "search_jobs(boolean,integer,text,text,text,boolean,uuid[],boolean,integer)", "count_jobs(boolean,integer,text,text,text,boolean,uuid[],boolean)", "get_job(uuid)")
