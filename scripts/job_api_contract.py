...
def _function_contract(connection: psycopg.Connection, signature: str, name: str) -> dict[str, Any]:
    ...
    if result_columns:
        columns = [
            {"name": column_name, "type": type_name, "order": order}
            for column_name, type_name, order in result_columns
        ]
    else:
        columns = [{"name": name, "type": return_type, "order": 1}]
...
def contract_for_connection(connection: psycopg.Connection) -> dict[str, Any]:
    return {
        "functions": {
            name: _function_contract(connection, signature, name)
...
def write_contract(connection: psycopg.Connection, output: Path) -> None:
    ...
    output.write_text(
        json.dumps(contract_for_connection(connection), indent=2) + "\n",
        encoding="utf-8",
    )
