...
export function countGlobeRoles(groups: DuplicateJobGroup[], points: GlobePoint[], visiblePostingIds: readonly string[]) {
  const mappedIds = new Set(points.map(point => point.posting_id));
  const mapped = groups.filter(group => [group.job, ...group.alternates].some(job => job.alive !== false && mappedIds.has(job.id))).length;
  const visible = partitionGlobeGroups(groups, visiblePostingIds).visible.length;
  return { total: groups.length, mapped, unmapped: groups.length - mapped, visible, outside: groups.length - visible };
}
