...
export function JobCard({ job, alternateCount = 0, openerRef, selectJob, children }: Props) {
  ...
  return <article data-posting-id={job.id} className="relative flex w-full flex-col gap-3 rounded-xl border bg-card p-4 text-card-foreground shadow-sm transition-colors hover:border-ring/50 focus-within:ring-2 focus-within:ring-ring">
    ...
    <div className="pointer-events-none flex items-start gap-3">
      <CompanyLogo company={job.company} className="size-12" />
      <div className="min-w-0 flex-1">
        <p className="truncate text-xs text-muted-foreground">{job.company}</p>
        <h2 className="mt-1 line-clamp-2 h-10 text-sm font-semibold leading-5" title={job.title}>{job.title}</h2>
      </div>
    ...
    <p className="pointer-events-none truncate text-xs text-muted-foreground" title={`${job.location ?? "Location unspecified"} · ${workMode(job.remote)}`}>{job.location ?? "Location unspecified"} · {workMode(job.remote)}</p>
    ...
    <div className="pointer-events-none mt-auto flex min-w-0 items-center gap-2 text-[11px] text-muted-foreground">
