...
export type Filter = "all" | "new-for-me" | "seen" | "saved" | "applied" | "rejected";
...
type RowProps = { job: JobSummary; openerRef: Opener; selectJob: (id: string | null) => void };
type ListProps = {
  filter: Filter; search: string; jobs: JobSummary[]; visible: JobSummary[]; loading: boolean; error: string;
  openerRef: Opener; selectJob: RowProps["selectJob"]; chooseFilter: (filter: Filter) => void;
  setSearch: (value: string) => void; reload: () => void;
};
type BodyProps = {
  detail: JobDetail | null; detailError: string; saving: boolean; rejecting: boolean; reason: string;
  setReason: (value: string) => void; setRejecting: (value: boolean) => void;
  changeStatus: (patch: StatusPatch) => Promise<void>;
};
type DrawerProps = BodyProps & { selected: string | null; openerRef: Opener; selectJob: RowProps["selectJob"] };

export function BoardHeader() { return <header className="border-b border-[#dce3dc] bg-white"><div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-5 sm:px-8">
...
export function JobsPanel({filter, search, jobs, visible, loading, error, openerRef, selectJob, chooseFilter, setSearch, reload}: ListProps) { return <section aria-label="Job search" className="rounded-2xl border border-[#dce3dc] bg-white shadow-sm">
        ...
        {!error && <div className="divide-y divide-[#e7ebe4]">{visible.map((job) => <JobRow job={job} key={job.id} openerRef={openerRef} selectJob={selectJob} />)}</div>}
      </section>; }

function ScoreReasons({reasons}: {reasons: JobDetail["reasons"]}) { if (!reasons.length) return null; return <section><h2 className="font-semibold">Why this score</h2><ul className="mt-3 space-y-3">{reasons.map((item) => <li key={item.factor} className="rounded-lg border p-3 text-sm"><span className="mb-1 block text-xs capitalize text-muted-foreground">{item.factor.replaceAll("_", " ")} · {item.assessment}</span>{item.detail}</li>)}</ul></section>; }

function DetailBody({detail, detailError, saving, rejecting, reason, setReason, setRejecting, changeStatus}: BodyProps) {
if (!detail) return <p className="p-6" role="status">{detailError || "Loading job…"}</p>;
return <div className="space-y-6 p-6">{detailError && <p role="alert" className="rounded-lg bg-red-50 p-3 text-sm text-red-800">{detailError}</p>}
        ...
        <ScoreReasons reasons={detail.reasons} />
...
export function JobDrawer({selected, openerRef, selectJob, ...bodyProps}: DrawerProps) { const {detail} = bodyProps; return <Sheet open={selected !== null} onOpenChange={(open) => { if (!open) selectJob(null); }}><SheetContent finalFocus={openerRef} className="overflow-y-auto data-[side=right]:w-full data-[side=right]:sm:max-w-2xl">
