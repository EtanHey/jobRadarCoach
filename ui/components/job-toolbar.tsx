...
export function JobToolbar({ jobs, options, onChange, onReset, canReset = false, actions, globeOpen = false }: {
  ...
  return <div data-job-toolbar>
    <div className={`hidden items-end gap-3 pb-2 ${globeOpen ? "xl:flex" : "md:flex"}`}><div className="grid min-w-0 flex-1 grid-cols-3 items-end gap-3 xl:grid-cols-8">{controls}</div><div className="flex shrink-0 items-center gap-1"><Button type="button" variant="ghost" disabled={!canReset} onClick={onReset}>Reset view</Button>{actions}</div></div>
