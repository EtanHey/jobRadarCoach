...
export function JobBoard() {
  ...
  function focusGlobeRow(id: string | null, source: "point" | "rail" = "rail") {
    ...
    if (rowId) requestAnimationFrame(() => {
      ...
      if (row && rail && rail.scrollHeight > rail.clientHeight) {
  ...
  return <div className={`bg-background text-foreground ${globeActive ? "board-globe-open" : "min-h-screen"}`}>
    <BoardHeader>{globeActive && <span role="status" title={connection} className="flex items-center gap-1 text-xs text-muted-foreground"><i aria-hidden="true" className={`size-2 rounded-full ${connection.startsWith("Live") ? "bg-emerald-500" : "bg-amber-500"}`} />{connection.startsWith("Live") ? "Live" : "Reconnecting…"}</span>}<ProfileDrawer onUpdated={requestRefresh} /></BoardHeader>
