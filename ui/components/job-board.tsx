...
export function JobBoard() {
  ...
  const globeMeta = <>{globeActive && globeCounts && <><span>{globeCounts.unmapped} {globeCounts.unmapped === 1 ? "role" : "roles"} not on globe</span><span>{globeCounts.mapped} {globeCounts.mapped === 1 ? "role" : "roles"} mapped{points.length > 5000 ? " · showing a sample of up to 5,000 postings" : ""}</span></>}{globeActive && !globe.data && <span role="status">Loading all posting locations…</span>}{(globe.failure || globeWarning) && <span role="alert" className="text-amber-700">{globeWarning || globe.failure} Use Globe to retry.</span>}</>;
