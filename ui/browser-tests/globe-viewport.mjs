...
try{for(const mobile of [false,true]){
 ...
 try{
  ...
  assert.equal(await page.getByText('1 role not on globe',{exact:true}).count(),1);assert.equal(await page.getByText('6 roles mapped',{exact:true}).count(),1);
