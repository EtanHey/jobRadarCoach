...
try{for(const mobile of [false,true]){
 ...
 try{
  ...
  await page.getByPlaceholder('Search title, company, or stack').fill('Duplicate');await page.waitForTimeout(250);const filtered=await parity([id(6)]);await page.getByText('2 mapped',{exact:true}).waitFor();await page.getByText('0 postings not on globe',{exact:true}).waitFor();
